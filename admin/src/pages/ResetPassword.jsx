...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.response?.data?.error?.message || "Failed to reset password. The token may be expired.");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      {success ? (
        <div className="space-y-6 text-center">
          ...
          <Link to="/login">
            <Button className="w-full h-12 font-medium bg-red-600 hover:bg-red-700">
              Go to Login
            </Button>
          </Link>
        </div>
